...
class PluginFetchFileListFromDrupalView extends AbstractFetchResourceListPlugin
{
    public function execute()
    {
        ...
        $url = $this->drupal_base_url . '/riprap_file_list?page=' . $page_number;
        ...
        if ($page_response->getStatusCode() == 200) {
            $file_list = (string) $page_response->getBody();
            $file_list = json_decode($file_list, true);
        }

        $empty_file_list_message = "PluginFetchFileListFromDrupalView retrieved an empty file list. " .
            "This probably means Riprap has finished checking all your files. However, if Riprap " .
            "shows this message the next time it runs, you should check to make sure " .
            "your \"Riprap file list\" Drupal View is working properly.";

        // Loop through all the media perform fixity event check.
        $num_file = count($file_list);
        if ($num_file == 0) {
            $this->setPageNumber($page_number, $num_file);
            if ($this->logger) {
                $this->logger->error($empty_file_list_message);
            }
            $output->writeln($empty_file_list_message);
            exit(1);
        }

        // Rename mid to fid? - Scratch that nvm
        // foreach ($media_list as $key => $field) {
        //     $media_list[$key]['mid'] = $media_list[$key]['fid'];
        // }
        ...
        foreach ($file_list as $file) {
            ...
            if ($this->settings['plugins.fetchdigest'] == 'PluginFetchDigestFromDrupal') {
                ...
                // the resource ID / URL cannot be found. (But, http responses are already logged in
                // getFileUrlFromFedora() so maybe we don't need to log here?)
                $file_drupal_uri = $this->getFileUriFromDrupal($file['fid']);
        ...
        if ($this->logger) {
            $this->logger->info("PluginFetchFileListFromDrupal executed");
        }
    ...
    */
    private function getFileUriFromDrupal($fid)
    {
        // Retrieve the media entity from Drupal.
        try {
            $url = $this->drupal_base_url . '/file/' . $fid . '?_format=json';
            $client = new \GuzzleHttp\Client();
            ...
            if ($code == 200) {
                return $body['uri'][0]['value'];
            } elseif ($code == 404) {
                return false;
            } else {
                if ($this->logger) {
                    $this->logger->error(
                        "PluginFetchFileListFromDrupal could not get File URI from Drupal.",
                        array(
    ...
    // */
    // private function getFileFitsChecksumFromDrupal($mid)
    // {
    //     // Retrieve the media entity from Drupal.
    //     try {
    ...
    //                 $file_url = $this->drupal_base_url . '/file/' . $file_id . '?_format=json';
    ...
    //                 return $file_body['field_fits_checksum'][0]['value'];
    //             }
    //         } elseif ($code == 404) {
    //             return false;
    //         } else {
    //             if ($this->logger) {
    //                 $this->logger->error(
    //                     "PluginFetchFileListFromDrupal could not get File FITS checksum from Drupal.",
    //                     array(
    //                         'HTTP response code' => $code
    //                     )
    //                 );
